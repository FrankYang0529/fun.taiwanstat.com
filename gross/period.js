...
var req_url = function(boxf, count, cb) {
  ...
  if(fs.existsSync('./year_gross/' + year + '_result.csv'))
    fs.unlinkSync('./year_gross/' + year + '_result.csv');

  request(boxf, function(err, resp, body){
    if (!err && resp.statusCode == 200) {
      $ = cheerio.load(body);

      fs.appendFileSync('./year_gross/' + year + '_result.csv', field.join(',') + '\n')
      ...
      movies.each(function(i, mov){
        ...
        fs.appendFileSync('./year_gross/' + year + '_result.csv', clip.join(',') + '\n');
        cb();
      })
    }else {
