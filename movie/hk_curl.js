...
var field = ['排名', '上週', '香港週末', '增率', '累積新台幣', '周次', '連結', '片名', '英文片名', 'Youtube', '時間', '海報', '上市', 'IMDB', 'IMDB_vote', 'imdb_ID'];

var myurl = 'http://hk.dorama.info/drama/d_box_office.php';

if(fs.existsSync('./hk_result.csv'))
  fs.unlinkSync('./hk_result.csv');
...
var req_url = function(myurl) {
  request(myurl, function(err, resp, body){
    if (!err && resp.statusCode == 200) {
      ...
      fs.appendFileSync('./hk_result.csv', field.join(', ') + '\n')
        ...
        if(i < 60) {
          if(ent.children[0].next && ent.children[0].next.attribs.class === 'td_dt_g') {
            ...
            if(!url.parse(href).protocol)
              var href = 'http://hk.dorama.info' + href;
                  ...
                  if (!err && resp.statusCode == 200) {
                    ...
                    request('https://www.googleapis.com/youtube/v3/search?part=snippet&q=' + name + '&key=AIzaSyCN7Fm5tDc3S1uIy1UPk9dURgYL3dreVmw', function(err, resp, utube){
                      if (!err && resp.statusCode == 200) {
                        ...
                        fs.appendFileSync('./hk_result.csv', line + '\n');
                      }
                    })
                  }
                })
